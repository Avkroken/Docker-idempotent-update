...
2. Pulls images for each service sequentially, with a retry mechanism (up to 3 attempts with linear backoff — 5s, then 10s).
...
| :--- | :--- | :--- |
| `MODE` | `both` | Must be `update`, `backup`, or `both`. Updates only run when `MODE` is `update` or `both`. |
