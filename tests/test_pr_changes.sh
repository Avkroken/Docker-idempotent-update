...
assert_yaml_field "$CI" "data['name']" "Python lint & tests" "ci.yml: workflow name matches current CI"
assert_yaml_field "$CI" "str('pull_request' in data[True])" "True" "ci.yml: triggered on pull_request"
assert_yaml_field "$CI" "str('merge_group' in data[True])" "False" "ci.yml: has no unused merge queue trigger"
...
assert_yaml_field "$CI" "str(set(data['jobs']) == {'lint', 'python'})" "True" "ci.yml: defines lint and python jobs"
assert_yaml_field "$CI" "data['jobs']['lint']['runs-on']" "ubuntu-latest" "ci.yml: lint job runs on ubuntu-latest"
assert_yaml_field "$CI" "data['jobs']['python']['runs-on']" "ubuntu-latest" "ci.yml: python job runs on ubuntu-latest"
assert_yaml_field "$CI" "str(any('ruff check src/' in str(s.get('run','')) for s in data['jobs']['lint']['steps']))" "True" "ci.yml: lint job runs ruff check"
assert_yaml_field "$CI" "str(any('python -m compileall .' in str(s.get('run','')) for s in data['jobs']['python']['steps']))" "True" "ci.yml: python job compiles sources"
